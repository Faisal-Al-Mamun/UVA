...
ll cap[1001][1001];
ll flowPassed[1001][1001];
vector<ll> g[1001];
ll parList[1001];
ll currentPathC[1001];

ll bfs(ll sNode, ll eNode)//breadth first search
{
    memset(parList, -1, sizeof(parList));
    memset(currentPathC, 0, sizeof(currentPathC));
    queue<ll> q;//declare queue vector
    q.push(sNode);
    parList[sNode] = -1;//initialize parlist’s source node
    currentPathC[sNode] = INT_MAX;//initialize currentpath’s source node
    ...
    while(!q.empty())// if q is not empty
    {
        ...
        for(ll i=0; i<g[currNode].size(); i++)
        {
            ...
            if(parList[to] == -1)
            {
                if((cap[currNode][to] - flowPassed[currNode][to]) > 0)
                {
                    parList[to] = currNode;
                    currentPathC[to] = min(currentPathC[currNode], cap[currNode][to] - flowPassed[currNode][to]);
                    if(to == eNode)
                    {
...
ll edmondsKarp(ll sNode, ll eNode)
{
    ll maxFlow = 0;
    while(true)
    {
        ll flow = bfs(sNode, eNode);
        if (flow == 0)
        {
            break;
        }
...
ll i=0, j=0, k=0, a, b, c, d, e, f, h, l, m, n, o, p, q, r, s, t=1, u, v, w, x, y, z, maxx=INT_MIN, minn=INT_MAX, ans=0, sum=0, flag=0, cnt=0;
...
int main()
{
    ...
    while(cin>>n && n)
    {
        ...
        cout<<"Network "<<t++<<endl;
        cout<<"The bandwidth is "<<edmondsKarp(a,b)<<".\n"<<endl;
        f0(i,n+5)
        {
            g[i].clear();
        }
