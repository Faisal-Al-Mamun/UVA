...
ll i=0, j=0, k=0, a, b, c, e, f, g, h, l, m, n, o, p, q, r, s, t=1, w, x, y, z, maxx=INT_MIN, minn=INT_MAX, ans=0, sum=0, flag=0, cnt=0;

vector<ll> G[1005];// adjacency list of graph
...
int main()
{
    ...
    while(cin>>n)
    {
        ...
        find_BR();
        cout<<BR.sz<<" critical links"<<endl;
        sort(all(BR));
        for(ll i = 0; i<BR.sz; i++)
        {
            cout<<BR[i].F-1<<" - "<<BR[i].S-1<<endl;
        }
        cout<<endl;
        BR.clear();
        for(ll i = 1; i<=n; i++)
